...
export async function GET(
  ...
  try {
    ...
    const res = await fetch(`${backendURL}/api/v1/businesses/${businessId}/available-pricing-plans`, {
      method: 'GET',
      headers: {
    ...
    const payload = JSON.parse(text) as { pricing_plans: BackendPricingPlanResponse[] }
    return NextResponse.json(
      {
        pricingPlans: payload.pricing_plans.map(toPricingPlan),
      } satisfies OwnerSelectablePricingPlansResponse,
      { status: res.status }
    )
  } catch (error) {
    console.error('Available pricing plans proxy error:', error)
    return NextResponse.json(
