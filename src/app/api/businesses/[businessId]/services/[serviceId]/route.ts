...
export async function PUT(
  ...
  try {
    ...
    const body = (await req.json()) as UpdateServicePayload
    const backendURL = getBackendBaseUrl()
    const res = await fetch(`${backendURL}/api/v1/businesses/${businessId}/services/${serviceId}`, {
      method: 'PUT',
      headers: {
    ...
    console.error('Service update proxy error:', error)
    return NextResponse.json(
