...
export async function PUT(
  ...
  try {
    ...
    const body = (await req.json()) as ReorderBusinessMediaPayload
    const backendURL = getBackendBaseUrl()

    const res = await fetch(`${backendURL}/api/v1/businesses/${businessId}/media/reorder`, {
      method: 'PUT',
      headers: {
    ...
    const media = (JSON.parse(text) as BackendBusinessMediaResponse[]).map(toBusinessMedia)
    return NextResponse.json(
      {
        message: 'Gallery order updated successfully.',
        media,
      } satisfies BusinessMediaReorderResponse,
      { status: res.status }
    )
  } catch (error) {
    console.error('Business media reorder proxy error:', error)
    return NextResponse.json(
