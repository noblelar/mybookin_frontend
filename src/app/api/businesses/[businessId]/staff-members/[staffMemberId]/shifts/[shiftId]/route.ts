...
export async function DELETE(
  ...
  {
    params,
  }: { params: Promise<{ businessId: string; staffMemberId: string; shiftId: string }> }
) {
  try {
    const { businessId, staffMemberId, shiftId } = await params
    ...
    const res = await fetch(
      `${backendURL}/api/v1/businesses/${businessId}/staff-members/${staffMemberId}/shifts/${shiftId}`,
      {
        method: 'DELETE',
        headers: {
    ...
    console.error('Staff shift delete proxy error:', error)
    return NextResponse.json(
