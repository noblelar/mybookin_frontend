...
import { toStaffShift } from '@/lib/server-staff'
...
export async function GET(
  ...
  try {
    ...
    const res = await fetch(
      `${backendURL}/api/v1/businesses/${businessId}/staff-members/${staffMemberId}/shifts`,
      {
        method: 'GET',
        headers: {
    ...
    const shifts = (JSON.parse(text) as BackendStaffShiftResponse[]).map(toStaffShift)
    return NextResponse.json({ shifts } satisfies StaffShiftListResponse, { status: res.status })
  } catch (error) {
    console.error('Staff shifts list proxy error:', error)
    return NextResponse.json(
...
export async function POST(
  ...
  try {
    ...
    const body = (await req.json()) as CreateStaffShiftPayload
    const backendURL = getBackendBaseUrl()
    const res = await fetch(
      `${backendURL}/api/v1/businesses/${businessId}/staff-members/${staffMemberId}/shifts`,
      {
    ...
    const shift = toStaffShift(JSON.parse(text) as BackendStaffShiftResponse)
    return NextResponse.json(
      {
        message: 'Shift created successfully.',
        shift,
      } satisfies StaffShiftMutationResponse,
      { status: res.status }
    )
  } catch (error) {
    console.error('Staff shift create proxy error:', error)
    return NextResponse.json(
      { message: 'Unexpected error.' } satisfies ApiErrorResponse,
      { status: 500 }
    )
