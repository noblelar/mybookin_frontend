...
export async function GET(
  ...
  try {
    ...
    const res = await fetch(
      `${backendURL}/api/v1/businesses/${businessId}/staff-members?include_inactive=${encodeURIComponent(includeInactive)}`,
      {
        method: 'GET',
        headers: {
    ...
    return NextResponse.json({ staffMembers } satisfies StaffListResponse, { status: res.status })
  } catch (error) {
    console.error('Staff members list proxy error:', error)
    return NextResponse.json(
