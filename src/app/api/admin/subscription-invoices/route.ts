...
export async function POST(req: NextRequest) {
  try {
    ...
    const res = await fetch(`${backendURL}/api/v1/admin/subscription-invoices`, {
      method: 'POST',
      headers: {
    ...
    const payload = JSON.parse(text) as { invoice: BackendSubscriptionInvoiceResponse }
    return NextResponse.json(
      {
        message: 'Invoice created successfully.',
        invoice: toSubscriptionInvoice(payload.invoice),
      } satisfies SubscriptionInvoiceCreateSuccessResponse,
      { status: res.status }
    )
  } catch (error) {
    console.error('Subscription invoice create proxy error:', error)
    return NextResponse.json(
