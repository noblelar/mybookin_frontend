...
export interface ServiceCardProps {
  ...
  price: string
  nextSlot?: string
  urgency?: string        // e.g. "Only 2 spots left"
  isPremium?: boolean
}
...
export default function ServiceCard({
  ...
  /* ─── Premium Partner card (horizontal layout on mobile) ─── */
  if (isPremium) {
    return (
      <div className="bg-white border border-[rgba(198,198,205,0.15)] shadow-sm overflow-hidden">
        <div className="flex items-stretch gap-0">
          {/* Small square image */}
          <div className="w-[110px] md:w-[140px] flex-shrink-0 overflow-hidden">
        ...
        {/* Fast Book button */}
        <button className="w-full py-2.5 bg-[#EFF4FF] text-[#0B1C30] text-xs font-black tracking-widest uppercase hover:bg-[#DCE9FF] transition-colors border-t border-[rgba(198,198,205,0.2)]">
          Fast Book
        </button>
      </div>
    )
  ...
  /* ─── Standard card ─── */
  return (
    <div className="bg-white border border-[rgba(198,198,205,0.15)] shadow-sm overflow-hidden group hover:shadow-md transition-shadow duration-200 cursor-pointer">
      {/* Image */}
      <div className="relative h-[200px] md:h-[195px] overflow-hidden">
        ...
        />
        {/* Desktop category badge (bottom-left) */}
        ...
        {/* Rating badge (top-right) — visible on mobile */}
        ...
        >
          <svg
            width="14" height="13" viewBox="0 0 24 22"
      ...
      <div className="px-3 md:px-4 pt-3 pb-3 flex flex-col gap-1.5">
        ...
        {/* Distance + next slot OR rating (desktop) */}
